  ...
  return (
    <div className="rounded-2xl border border-cyan-500/30 bg-slate-950/80 p-2.5 shadow-xl shadow-black/35 backdrop-blur-xl">
      <div className="flex items-center gap-2">
        <p className="px-1 text-[10px] font-black uppercase tracking-[0.16em] text-cyan-200">Music</p>
        <button
          onClick={() => {
            void toggleEnabled();
          }}
      ...
      {expanded && (
        <div className="mt-2 flex items-center gap-2 border-t border-slate-700/70 pt-2">
          ...
          />
          <span className="w-8 text-right text-[10px] font-mono text-slate-300">{Math.round(volume * 100)}</span>
        </div>
      )}
    </div>
  );
